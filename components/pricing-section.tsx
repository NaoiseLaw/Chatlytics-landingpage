...
export function PricingSection() {
  ...
  return (
    <section className="w-full px-5 overflow-hidden flex flex-col justify-start items-center my-0 py-8 md:py-14">
      ...
      <div className="self-stretch px-5 flex flex-col md:flex-row justify-start items-start gap-4 md:gap-6 mt-6 max-w-[1320px] mx-auto flex-wrap">
        {pricingPlans.map((plan) => (
          <div
            key={plan.name}
            className={`flex-1 min-w-[280px] p-4 overflow-hidden rounded-xl flex flex-col justify-start items-start gap-6 ${plan.popular ? "bg-primary shadow-[0px_4px_8px_-2px_rgba(0,0,0,0.10)]" : "bg-gradient-to-b from-gray-50/5 to-gray-50/0"}`}
            ...
            <div className="self-stretch flex flex-col justify-start items-start gap-6">
              ...
              >
                <div className="px-1.5 flex justify-center items-center gap-2">
                  <span
                    className={`text-center text-sm font-medium leading-tight ${plan.name === "Free" ? "text-gray-800" : plan.name === "Pro" ? "text-primary" : plan.name === "Ultra" ? "text-zinc-950" : "text-accent-foreground"}`}
                  >
                    {plan.buttonText}
                  </span>
                </div>
